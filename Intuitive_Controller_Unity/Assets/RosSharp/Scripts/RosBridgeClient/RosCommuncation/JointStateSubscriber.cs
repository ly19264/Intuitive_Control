...
//          for (int i = 0; i < message.name.Length; i++)
            for (int i = 0; i < 7; i++)    //Changed the maximum iteration of the loop to the no. of joints in the arm
            {
                //index = JointNames.IndexOf(message.name[i]);
                index = i;   // Changed the index to joint no. as the IndexOf method is not working for franka panda  
                if (index != -1)
